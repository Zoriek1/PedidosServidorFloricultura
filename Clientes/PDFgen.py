...
def descobrir_servidor():
    ...
    Retorna URL do servidor ou None se não encontrado
    """
        ...
        while time.time() - start_time < 10:
            try:
                ...
                if message.get('service') == 'plante-uma-flor-server':
                    ...
                    print(f"✅ Servidor encontrado via broadcast: {server_url}")
                    sock.close()
                    return server_url
...
# Cache global da URL do servidor (descoberta uma vez por sessão)
_SERVER_URL_CACHE = None

def get_server_url():
    """Retorna URL do servidor (usa cache se já descobriu antes)"""
    global _SERVER_URL_CACHE
    if _SERVER_URL_CACHE is None:
        _SERVER_URL_CACHE = descobrir_servidor()
...
class App:
    ...
    def enviar_pedido_para_painel(self, pedido: dict):
        """Envia pedido para o servidor Flask via POST (com descoberta automática)"""
        try:
            # Descobrir servidor automaticamente
            server_url = get_server_url()
            
            if not server_url:
                print("⚠️ Servidor não encontrado na rede. Pedido salvo localmente.")
                return
            
            # Montar dados para envio conforme especificação da API v2.0
            dados_envio = {
                ...
                # Campos novos da v2.0
                "telefone_cliente": pedido.get("telefone_cliente", ""),
                "tipo_pedido": pedido.get("tipo_pedido", "Entrega"),
                "endereco": pedido.get("endereco", ""),
                "observacoes": pedido.get("obs", "")
            }
            
            # Enviar requisição POST para o servidor
            api_url = f"{server_url}/api/pedidos"
            print(f"📤 Enviando pedido para: {api_url}")
            ...
            print(f"✅ Pedido #{pedido_id} enviado ao painel v2.0 com sucesso!")
            print(f"   Servidor: {server_url}")
            
        except requests.exceptions.ConnectionError:
            print(f"⚠️ Erro de conexão: Servidor não acessível")
            ...
            print(f"   Pedido salvo localmente no banco de dados")
